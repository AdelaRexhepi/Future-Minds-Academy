...
function render(){
    ...
    if((catObj.x + catObj.width) > mouseObj.x
        ...
        && (catObj.y+10)< (mouseObj.y + mouseObj.height)) {
            mousePos();
            if(ticTimer != 0){points++} else(mouseObj.x = 3000)
...
function timer(){
    if(ticTimer <= 0){ clearInterval(timeRunner);}
    else{ticTimer--;}
}

const renederFrame = setInterval(render,1); // thirre render cdo 1/1000 second
